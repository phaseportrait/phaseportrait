...
#     def __init__(self, portrait, funcion, *, precision=0.01, xprecision=None, yprecision=None, zprecision=None, offset=0, density=50, xRange=None, yRange=None, zRange=None, dF_args=None, xcolor='r', ycolor='b', zcolor='g', bgcolor='w', alpha=0, polar=False):
#         """Creates an instance of Nullcline3D
...
#         yprecision : float, optional
#             For a different precision value only in the y axis, by default `precision`
#         zprecision : float, optional
#             For a different precision value only in the z axis, by default `precision`
#         offset : float, optional
#             If you want, for instance, a twoclide, by default 0
#         density : int, optional
#             Number of inner divisions on the x axis and y axis, by default 50
...
#         yRange : Union[float,list], optional
#             The range in which the nullclines are calculated, by default `portrait.Range[1]`
#         zRange : Union[float,list], optional
#             The range in which the nullclines are calculated, by default `portrait.Range[2]`
#         dF_args : dict, optional
#             If necesary, must contain the kargs for the `dF` funcion, by default None
#         xcolor : str, optional
#             X nullcline color, by default 'r'
...
#             Y nullcline color], by default 'b'
#         zcolor : str, optional
#             Z nullcline color], by default 'g'
#         bgcolor : str, optional
#             Background color, by default 'w'
#         alpha : int, optional
#             Opacity of the background, by default 0
...
#         self.xprecision = xprecision if xprecision is not None else precision
#         self.yprecision = yprecision if yprecision is not None else precision
#         self.zprecision = zprecision if zprecision is not None else precision
#         self.offset = offset
#         self.xcolor = to_hex(xcolor)
#         self.ycolor = to_hex(ycolor)
#         self.zcolor = to_hex(zcolor)
#         self.bgcolor = bgcolor
#         self.density = density
#         self.alpha = alpha
#         self.polar = polar
...
#         _x = np.linspace(*self.portrait.Range[0,:], self.density)
#         _y = np.linspace(*self.portrait.Range[1,:], self.density)
#         _z = np.linspace(*self.portrait.Range[2,:], self.density)
        
        
        
#         _xdF = np.zeros([self.density, self.density, self.density])
#         _ydF = np.zeros([self.density, self.density, self.density])
#         _zdF = np.zeros([self.density, self.density, self.density])
...
#                 for j,yy in enumerate(_y):
#                     for k,zz in enumerate(_z):
#                         _xdF[k,j,i], _ydF[i,k,j], _zdF[j,i,k] = self.funcion(xx,yy,zz, **self.dF_args)
                        
                        
#         _x, _y, _z = np.meshgrid(_x, _y, _z)
        
#         xct = axis.contourf(_x, _y, _z, _xdF, levels=[-self.xprecision + self.offset, self.xprecision + self.offset], colors=[self.xcolor], extend='neither')
#         yct = axis.contourf(_x, _y, _z, _ydF, levels=[-self.yprecision + self.offset, self.yprecision + self.offset], colors=[self.ycolor], extend='neither')
#         zct = axis.contourf(_x, _y, _z, _zdF, levels=[-self.zprecision + self.offset, self.zprecision + self.offset], colors=[self.zcolor], extend='neither')
         
        
#         xct.cmap.set_over(self.bgcolor, alpha=self.alpha)
#         yct.cmap.set_over(self.bgcolor, alpha=self.alpha)
#         zct.cmap.set_over(self.bgcolor, alpha=self.alpha)
#         xct.cmap.set_under(self.bgcolor, alpha=self.alpha)
#         yct.cmap.set_under(self.bgcolor, alpha=self.alpha)
#         zct.cmap.set_under(self.bgcolor, alpha=self.alpha)
